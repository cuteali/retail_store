module Weixinpay
  class << self
    ...
    def set_sign_params request_options = {}
      [
        "appid=#{request_options[:appid]}", 
        "mch_id=#{request_options[:mch_id]}", 
        "nonce_str=#{request_options[:nonce_str]}", 
        "body=#{request_options[:body]}", 
        "out_trade_no=#{request_options[:out_trade_no]}", 
        "total_fee=#{request_options[:total_fee]}",
        "spbill_create_ip=#{request_options[:spbill_create_ip]}", 
        "notify_url=#{request_options[:notify_url]}", 
        "trade_type=#{request_options[:trade_type]}", 
        "time_start=#{request_options[:time_start]}", 
        "time_expire=#{request_options[:time_expire]}"
      ]
    end

    def notify_result result = {}
      "<xml>
        <return_code><![CDATA[#{result[:return_code]}]]></return_code>
        <return_msg><![CDATA[#{result[:return_msg]}]]></return_msg>
      </xml>"
    end 
  end
end
