class Product < ActiveRecord::Base
  ...
  def self.init_shop_products(shop)
    Product.normal.sorted.each do |p|
      ...
      sub_category = shop.sub_categories.normal.find_by(name: p.sub_category.try(:name))
      detail_category = shop.detail_categories.normal.find_by(name: p.detail_category.try(:name))
